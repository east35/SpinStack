...
**IMPORTANT:** If you're already running SpinStack locally (not in Docker), you'll need separate environment configurations for local vs Docker deployment.
...
2. Edit `.env` with your Docker settings:
   ```env
   DISCOGS_CONSUMER_KEY=your_actual_key
   DISCOGS_CONSUMER_SECRET=your_actual_secret
   SESSION_SECRET=$(openssl rand -base64 32)
   FRONTEND_URL=http://YOUR_SYNOLOGY_IP:6200
   NEXT_PUBLIC_API_URL=http://YOUR_SYNOLOGY_IP:3001
   ```

3. **Important:**
   - Update `YOUR_SYNOLOGY_IP` with your actual Synology IP address (e.g., `192.168.1.100`)
   - If you have an existing `.env` for local development, back it up first (e.g., `cp .env .env.local`)
   - The `.env` file in the project root is used by Docker Compose for container environment variables

### 4. Update Discogs OAuth Callback

1. Go to [Discogs Developer Settings](https://www.discogs.com/settings/developers)
2. Edit your application
3. Update the **Callback URL** to: `http://YOUR_SYNOLOGY_IP:6200/auth/callback`
...
- **Frontend:** `http://YOUR_SYNOLOGY_IP:6200`
- **Backend API:** `http://YOUR_SYNOLOGY_IP:3001`

## First-Time Setup

1. Open `http://YOUR_SYNOLOGY_IP:6200` in your browser
...
If ports 6200 or 3001 are already in use, edit `docker-compose.yml`:
...
3. Ensure ports 6200 and 3001 are allowed

### OAuth Callback Error

1. Verify `FRONTEND_URL` in `.env` matches your Synology IP
2. Verify Discogs callback URL matches exactly: `http://YOUR_IP:6200/auth/callback`
